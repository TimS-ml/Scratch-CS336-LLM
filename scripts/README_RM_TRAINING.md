...
pip install torch transformers datasets scikit-learn hydra-core
...
Edit `scripts/configs/rm_training.yaml`:

编辑 `scripts/configs/rm_training.yaml`：
...
deepspeed: scripts/configs/deepspeed/zero2.json  # For 2-8 GPUs
# or
deepspeed: scripts/configs/deepspeed/zero3.json  # For very large models
bf16: true
```
...
Edit `clean_llm/train/rm_train.py` to add custom loss:

编辑 `clean_llm/train/rm_train.py` 添加自定义损失：
...
```yaml
# In scripts/configs/rm_training.yaml
...
**See Also / 另请参阅:**
- Full DeepSpeed guide: `scripts/configs/deepspeed/README.md`
- Configuration files: `scripts/configs/deepspeed/zero*.json`
...
Modify `RMDataset` in `clean_llm/train/rlhf_datasets.py`:

修改 `clean_llm/train/rlhf_datasets.py` 中的 `RMDataset`：
...
├── clean_llm/
│   └── train/
│       ├── rm_train.py           # Main RM training module
│       └── rlhf_datasets.py      # Dataset classes
├── scripts/
...
- Check the troubleshooting section
  查看故障排除部分
- Review code comments in `clean_llm/train/rm_train.py`
  查看 `clean_llm/train/rm_train.py` 中的代码注释
